...
if [[ $# -lt 1 ]]; then
  ...
  echo "	output: https://github.com/SoftFever/OrcaSlicer/releases/download/2.2.53.3/SuperSlicer_2.2.53.3_linux64_20201005.zip"
  echo
...
if [[ ! -e "$baseDir/latestReleaseInfo.json" ]]; then
  ...
  curl -SsL https://api.github.com/repos/SoftFever/OrcaSlicer/releases/latest > $baseDir/latestReleaseInfo.json
...
if [[ $# -gt 1 ]]; then
  ...
  if [[ ! -e "$baseDir/releases.json" ]]; then
    curl -SsL https://api.github.com/repos/SoftFever/OrcaSlicer/releases > $baseDir/releases.json
  fi
...
if [[ "$1" == "url" ]]; then
  ...
  echo "${releaseInfo}" | jq -r '.assets[] | .browser_download_url | select(test("OrcaSlicer_Linux_V.+\\.AppImage"))'

elif [[ "$1" == "name" ]]; then

  echo "${releaseInfo}" | jq -r '.assets[] | .name | select(test("OrcaSlicer_Linux_V.+\\.AppImage"))'
  ...
  echo "${allReleases}" | jq --arg VERSION "${VER^}" -r 'first(.[] | .assets[] | .browser_download_url | select(test("OrcaSlicer_Linux_" + $VERSION + ".*\\.AppImage")))'

elif [[ "$1" == "name_ver" ]]; then
   
  echo "${allReleases}" | jq --arg VERSION "${VER^}" -r 'first(.[] | .assets[] | .name | select(test("OrcaSlicer_Linux_" + $VERSION + ".*\\.AppImage")))'
